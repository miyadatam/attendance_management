...
class LineLoginController extends Controller
{
  ...
  public function handleProviderCallback(Request $request)
  {
    ...
    //  ログイン処理(accesToken後ほど)
    $user = User::firstOrNew(['line_user_id' => $user_info->userId]);
    if(!$user->exist){
      $user->fill(['username' => $user_info->displayName])->save();
    }

    $user->fill(['line_img' => $user_info->pictureUrl])->save();
